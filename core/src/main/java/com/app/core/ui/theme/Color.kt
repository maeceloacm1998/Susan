package com.app.core.ui.theme

import androidx.compose.ui.graphics.Color

val Primary = Color(0xFF001E27)
val Secondary = Color(0xFFFF4E4E)
val BrowLight = Color(0xFFFFB601)
val BrowDark = Color(0xFFD99734)
val Background = Color(0xFFF7F7FC)
val RedDark = Color(0xFFB32457)
val SoftBlack = Color(0xFF1C1C1C)
val GrayDark = Color(0xFF737373)
val GrayLight = Color(0xFFD8D8D8)
val Success = Color(0xFF28A745)
val Error = Color(0xFFF34455)
val SuccessLight = Color(0x3228A745)
val BackgroundUserChat = Color(0xffe1e1e6)
val GoogleMapsIconColor = Color(0xff2d6a45)
val WazeBackgroundColor = Color(0xff39c2d8)